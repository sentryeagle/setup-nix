...
# Copyright 2019 Cachix
# Copyright 2025 SentryEagle
#
...
if [[ $INPUT_SET_CURRENT_USER_AS_TRUSTED_USER == 'true' ]]; then
...
# Get Nix latest version
# See: https://stackoverflow.com/a/51446831
nix_latest_version=$(git -c 'versionsort.suffix=-' ls-remote --exit-code --refs --sort='version:refname' --tags https://github.com/nixos/nix '*.*.*' | awk -F/ 'END{print$NF}')

# There is --retry-on-errors, but only newer curl versions support that
curl_retries=5
while ! curl -sS -o "$workdir/install" -v --fail -L "${INPUT_INSTALL_URL:-https://releases.nixos.org/nix/nix-${nix_latest_version}/install}"
do
  sleep 1
  ((curl_retries--))
